...
### 8.4 **redis过期策略和内存淘汰机制( **如何保证 redis 中的数据都是热点数据？ )
...
#### 8.5.3 redis缓存穿透、缓存击穿、缓存雪崩区别和解决方案
...
### 8.6 redis持久化：RDB（快照，容易丢失数据）  / AOF（生成文件大， 数据恢复速度慢）
