...
- newSingleThreadExecutor(单线程的线程池) -- LinkedBlockingQueue（链表有界队列）
- newFixedThreadPool (固定数目线程的线程池) -- LinkedBlockingQueue（链表有界队列）
- newCachedThreadPool(可缓存线程的线程池) -- SynchronousQueue（同步队列）
- newScheduledThreadPool(定时及周期执行的线程池) -- DelayQueue（延迟队列）
...
作者：Jay_huaxiao
链接：https://juejin.im/post/6844903889678893063
来源：掘金
